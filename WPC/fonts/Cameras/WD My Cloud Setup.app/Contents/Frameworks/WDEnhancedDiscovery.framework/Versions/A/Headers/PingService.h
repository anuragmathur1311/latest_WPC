...
#pragma mark * PingService

// The PingService class is a very simple class that lets you send and receive pings.

@protocol PingServiceDelegate;
...
@interface PingService : NSObject
{
    ...
    id<PingServiceDelegate>  _delegate;
    uint16_t                _identifier;                            // host byte order
    uint16_t                _nextSequenceNumber;                    // host byte order
}

+ (PingService *)pingWithHostName:(NSString *)hostName;        // chooses first IPv4 address
+ (PingService *)pingWithHostAddress:(NSData *)hostAddress;    // contains (struct sockaddr)

@property (nonatomic, assign, readwrite) id<PingServiceDelegate> delegate;
...
- (void)sendPingWithData:(NSData *)data;
    ...
    // ICMP header.
    //
    // Do not try to send a ping before you receive the -PingService:didStartWithAddress: delegate 
    // callback.
...
- (void)pingService:(PingService *)pinger didStartWithAddress:(NSData *)address;
    // Called after the PingService has successfully started up.  After this callback, you 
    // can start sending pings via -sendPingWithData:
    
- (void)pingService:(PingService *)pinger didFailWithError:(NSError *)error;
    // If this is called, the PingService object has failed.  By the time this callback is 
    // called, the object has stopped (that is, you don't need to call -stop yourself).

// IMPORTANT: On the send side the packet does not include an IP header. 
// On the receive side, it does.  In that case, use +[PingService icmpInPacket:] 
// to find the ICMP header within the packet.

- (void)pingService:(PingService *)pinger didSendPacket:(NSData *)packet;
    // Called whenever the PingService object has successfully sent a ping packet. 
    
- (void)pingService:(PingService *)pinger didFailToSendPacket:(NSData *)packet error:(NSError *)error;
    // Called whenever the PingService object tries and fails to send a ping packet.

- (void)pingService:(PingService *)pinger didReceivePingResponsePacket:(NSData *)packet;
    // Called whenever the PingService object receives an ICMP packet that looks like 
...
- (void)pingService:(PingService *)pinger didReceiveUnexpectedPacket:(NSData *)packet;
    // Called whenever the PingService object receives an ICMP packet that does not 
